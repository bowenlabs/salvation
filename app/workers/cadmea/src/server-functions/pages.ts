...
export const getPage = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data: id }) => {
    await requireAuthOrThrow();
    return (await pagesApi()).findByID(id);
  });
...
export const createPage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: input }) => {
    ...
    return (await pagesApi()).create(input as any);
  });
...
export const updatePage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return (await pagesApi()).update(data.id, data.values as any);
  });
...
export const deletePage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: id }) => {
    ...
    await requireSameOriginOrThrow();
    await checkWriteRateLimit(session);
    return (await pagesApi()).deleteByID(id);
  });
